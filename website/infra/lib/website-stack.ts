...
export interface WebsiteStackProps extends cdk.StackProps {
  /**
   * Custom domain (e.g. "harkformac.com"). Requires the Route 53 hosted
...
/**
 * Static site on S3 + CloudFront, the same shape as the sibling sites: a
 * private bucket that only CloudFront can read, clean URLs via a CloudFront
 * Function, and a `/downloads` prefix holding the signed dmgs that the
 * Homebrew cask and the download button fetch.
 */
export class WebsiteStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: WebsiteStackProps) {
    ...
    const siteBucket = new s3.Bucket(this, "SiteBucket", {
      ...
      enforceSSL: true,
      // Releases live under /downloads. Losing the bucket would break every
      // pinned cask version.
...
`),
    ...
    const distribution = new cloudfront.Distribution(this, "Distribution", {
      ...
      additionalBehaviors: {
        ...
        // and a short TTL so a fresh release is picked up promptly.
        "/downloads/*": {
          origin: origins.S3BucketOrigin.withOriginAccessControl(siteBucket),
          viewerProtocolPolicy: cloudfront.ViewerProtocolPolicy.REDIRECT_TO_HTTPS,
          cachePolicy: new cloudfront.CachePolicy(this, "DownloadsCache", {
            comment: "Short TTL so a new release is served quickly",
      ...
      errorResponses: [
        // S3 behind Origin Access Control answers 403, not 404, for a
        // missing key (the distribution has no ListBucket permission), so
        // both map to the not-found page.
