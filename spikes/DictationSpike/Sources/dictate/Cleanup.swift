...
struct TranscriptCleaner: Sendable {
    ...
    ///   HARK_CLEANUP=0            disable entirely
    ///   HARK_CLEANUP_URL          base URL (default http://localhost:1234/v1)
    ///   HARK_CLEANUP_MODEL        model id (default: first model the server lists)
    ///   HARK_CLEANUP_KEY          bearer token, if the endpoint needs one
    ///   HARK_CLEANUP_TIMEOUT_MS   hard budget (default 3000)
    static func fromEnvironment() async -> TranscriptCleaner? {
        let env = ProcessInfo.processInfo.environment
        if env["HARK_CLEANUP"] == "0" {
            print("[hark] cleanup: disabled (HARK_CLEANUP=0) — raw transcripts will paste.")
            return nil
        }
        let base = env["HARK_CLEANUP_URL"] ?? "http://localhost:1234/v1"
        guard let baseURL = URL(string: base) else {
            print("[hark] cleanup: invalid HARK_CLEANUP_URL '\(base)' — disabled.")
            return nil
        }
        let timeoutMs = Int(env["HARK_CLEANUP_TIMEOUT_MS"] ?? "") ?? 3000
        let apiKey = env["HARK_CLEANUP_KEY"]

        // Probe /models: confirms the server is up and supplies a default
        // model id. A dead server just means we start with cleanup off.
        var model = env["HARK_CLEANUP_MODEL"]
        if model == nil {
        ...
        guard let model else {
            print("""
                [hark] cleanup: no server at \(base) (and no HARK_CLEANUP_MODEL set) —
                       raw transcripts will paste. Start LM Studio/Ollama or set
                       HARK_CLEANUP_URL to enable cleanup.
                """)
            return nil
        }
        print("[hark] cleanup: \(model) @ \(base) (fail-open, \(timeoutMs) ms budget)")
    ...
    func clean(_ text: String) async -> Outcome {
        ...
        // 0 / 0.7 s on the same model). Default it off; HARK_CLEANUP_REASONING
        // overrides ("default" omits the parameter for servers that reject it
        // — a rejection fails open anyway).
        let reasoning = ProcessInfo.processInfo.environment["HARK_CLEANUP_REASONING"] ?? "none"
